  ...
  async create({ company, user, contact, address }) {
    ...
    try {
      ...
      if (companyInsertResult.affectedRows === 0) {
        await conn.rollback()
        return { error: `failureCompanyInsertion` }
      }
  ...
  async update(id_company, { name_company, cnpj_company, more_information }) {
    ...
    try {
      ...
      if (companyUpdateResult.affectedRows === 0) {
        return { error: `failureCompanyUpdate` }
      }

      return { message: `successCompanyUpdate` }
    } catch (error) {
      console.error(error)
      if (error?.message?.includes(`Duplicate`)) {
        return { error: `errorDuplicateRegistration` }
  ...
  async delete(id_company) {
    ...
    try {
      ...
      if (companyCompanyUpdateResult.affectedRows === 0) {
        return { error: `failureCompanyDeletion` }
      }

      return { message: `successCompanyDeletion` }
    } catch (error) {
      console.log(error)
      throw error
    } finally {
