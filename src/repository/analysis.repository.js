  ...
  async create(analysis) {
    ...
    try {
      ...
      if (analysisInsertResult.affectedRows === 0) {
        return { error: `failureAnalysisInsertion` }
      }

      return { message: `successAnalysisInsertion` }
    } catch (error) {
      console.error(error)
  ...
  async delete(id_analysis) {
    ...
    try {
      ...
      if (analysisUpdateResult.affectedRows === 0) {
        return { error: `failureAnalysisDeletion` }
      }

      return { message: `successAnalysisDeletion` }
    } catch (error) {
      console.error(error)
      throw error
    } finally {
