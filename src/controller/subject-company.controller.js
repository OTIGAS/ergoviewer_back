...
export default class SubjectCompanyController {
  create() {
    return async (req, res) => {
      ...
      try {
        const response = await subjectCompanyRepository.create({ company, user, contact, address })
        if (response.erro) {
          return RespostaErro(400, res, req, response)
        } else {
  ...
  list() {
    return async (req, res) => {
      ...
      try {
        const response = await subjectCompanyRepository.list({
          name_subject: name_subject || '',
          cnpj_subject: cnpj_subject || '',
  ...
  update() {
    return async (req, res) => {
      ...
      try {
        const response = await subjectCompanyRepository.update(id_subject, company)
        if (response.erro) {
          return RespostaErro(400, res, req, response)
        } else {
          return RespostaSucesso(200, res, req, response)
